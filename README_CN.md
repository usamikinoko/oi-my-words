...
oi-my-words 需要在请求中明确点名本技能才会启用，不依赖固定的斜杠命令，也不会仅凭通用措辞自动触发。**中文请求与英文请求都能触发**——真正起作用的是请求里出现技能名 oi-my-words，与请求所用的语言无关。把需要改写的文章草稿粘贴给模型，并在同一条请求里点名技能，例如"请遵循 oi-my-words 规则为我改写这段文字"；写成英文同样可以，例如"Rewrite this draft following the oi-my-words guidelines."。只写"去 AI 味"、"改写"、"润色"而不点名技能时不会触发；当意图明显是去除 AI 味、但用户未点名时，模型会先询问是否按 oi-my-words 规范处理，而不是自行套用。需要注意的是，本技能只处理已有文本的改写，不会改动标题与章节结构，也不会把用户原有的列表强行转换为段落；如果你需要从零撰写一篇新文章并规划章节结构，请点名姊妹技能 oi-my-words-create，例如"请基于 oi-my-words-create 规则为我编写一篇 Markdown 文档，要写的主题是……"。
